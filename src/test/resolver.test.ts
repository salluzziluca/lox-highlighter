...
function resolve(source: string): Map<string, string> {
	const tokens = new Scanner(source).scan();
	const stmts = new Parser(tokens).parse();
	return new Resolver().resolve(stmts);
}

function get(map: Map<string, string>, line: number, column: number): string | undefined {
...
suite('Resolver', () => {
	...
	suite('programa completo', () => {
		...
		test('error de parseo devuelve mapa vacío', () => {
			const tokens = new Scanner('var x = ;').scan();
			try {
				const stmts = new Parser(tokens).parse();
				new Resolver().resolve(stmts);
			} catch {
				assert.ok(true);
			}
		});
